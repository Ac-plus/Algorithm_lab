...
问题描述：高松灯喜欢收集石头。某天她想把石头送给千早爱音，但书包容量不足，于是只能认真选取几个。设她共有 $n$ 个石头，它们的重量和价值分别为 $w[i]$ 和 $p[i]$ ；书包的容量为 $c$ 。高松灯应该选取哪些石头装进书包才能使总价值最大？需要以 $n$ 元组的形式给出最优装法，同时求出在此最优解下的最大价值。
...
### 2.2 矩阵乘法链问题
问题描述：冰川纱夜上大学后学业繁忙，很少有时间陪妹妹冰川日菜。某天，日菜来找纱夜出去玩，但纱夜正忙着写线代课布置的一道题：给定 $q$ 个矩阵，求出它们连乘的结果。即使身为优等生的纱夜也被矩阵乘法的复杂计算所困扰。然而，身为天才的日菜扫了一眼题目就对姐姐说：“我有一个很 ｢噜｣♪ 的想法，只要改变矩阵乘法的顺序，就能大大减少计算的次数哦！这样姐姐就能赶紧写完陪我出去玩了！”请实现日菜所说的方法，即：已知这 $q$ 个矩阵的行列数，求使得元素间乘法次数最少的矩阵乘法顺序以及这一最少次数。
